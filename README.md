# Data Source
This data was directly sourced from Kaggle under the "COVID-19 Dataset" updated daily. This dataset's data mainly comes from  Worldometer, a website tracking various global statistics in real time like population and money spent in various sectors. The data also comes from the COVID-19 Data Repository by the Center for Systems Science and Engineering (CSSE) at Johns Hopkins University. However, this repository has ceased collecting data on March 10, 2023, meaning that the current updated data comes from the previously mentioned Worldometer.

# Application Domain
This project primarily focuses on the domain of public health. This domain is used for the tracking of COVID-19 cases across every country, including their mortality rate, total number infected and diseased, and the recovery rate of each country. The final result is a system desgined to supported properly structured and neat storage, as well as analysis of trends over time and across regions. This domain is relevant to public health specialists, researchers, and politicians determining how severe the pandemic is. 

# High Level Goals
## 1. Structured Data Storage
Store data in a simple and efficent manner that helps ensure data integrity and allows researchers to access the needed data quickly.
## 2. Analysis Across Countries
Assist researchers in comparing various statistics on the pandemic between countries and regions. This will be useful in finding trends within the data, such as what countries have the highest mortality rate.
## 3. Support Informed Policy Decisions
Build a clean and easy-to-access data model which can be used to inform analysis and research into potential decisions on how to react to the pandemic. 

# User Groups
Public Health Analyst- Make conclusions on the world's health levels based on the data
Researcher- Perform trend analysis across countries
Administrator- Manages the data structure and ensures it is accurately updated
